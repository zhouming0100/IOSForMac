...
@property (strong, nonatomic) BlogTableView *tableView;
//@property (strong, nonatomic) BlogViewPresenter *presenter;
@property (strong,nonatomic) id<BlogViewPresenterCallBack> presenter;
...
+ (instancetype)instanceWithPresenter:(id<BlogViewPresenterCallBack>)presenter {
    return [[BlogViewController alloc] initWithPresenter:presenter];
}

- (instancetype)initWithPresenter:(id<BlogViewPresenterCallBack>)presenter {
    if (self = [super init]) {
        
        self.tableView = [[BlogTableView alloc] initWithFrame:[UIScreen mainScreen].bounds style:UITableViewStylePlain];
...
- (void)fetchDataWithCompletionHandler:(NetworkCompletionHandler)completionHander {
    [self.presenter fetchDataWithCompletionHandler:completionHander];
}
